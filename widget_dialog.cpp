...
{
	settings& e1 = settings::root.gr("Localization", "Локализация").gr("General", "Общие").gr("Language", "Язык");
	e1.add("English", "Английский", locale, 0);
	e1.add("Russian", "Русский", locale, 1);
}
...
//{
//
//	const char* getid(const menu* m, int value)
//	{
//		auto pm = m->find(value);
//		if(!pm)
//			return "";
...
//	void write(io::writer& file, void* param) override
//	{
//		for(auto e = control::plugin::first; e; e = e->next)
//		{
...
//				continue;
//			file.open(id);
//			file.set("Docking", getid(docking_values, e->type));
//			file.set("Disabled", e->disabled ? "true" : "false");
//			auto p = e->instance->getparams();
//			if(p)
//			{
//				for(; *p; p++)
//				{
//					if((p->flags&ColumnPersistent) == 0)
//						continue;
//					auto pi = p->identifier;
//					if(!pi || pi[0]==0)
//						continue;
//					file.set(pi, e->instance->geti(p->id));
//				}
//			}
//			file.close(id);
//		}
...
//			return;
//		auto e = getcontrol(n.parent->name);
//		if(!e)
//			return;
//		int id = e->findparam(n.name);
//		if(!id)
//			return;
//		e->seti(id, value);
//	}
//
//	control::plugin* findsg(const char* name)
//	{
//		for(auto e = control::plugin::first; e; e = e->next)
//		{
//			auto p = e->identifier;
...
//		if(!n.parent || !n.parent->parent)
//			return;
//		auto e = findsg(n.parent->name);
//		if(!e)
//			return;
//		if(szcmpi(n.name, "Docking") == 0)
//		{
//			auto p = docking_values[0].find(value);
//			if(p)
//				e->type = p->id;
//		}
//		else if(szcmpi(n.name, "Disabled") == 0)
//			e->disabled = (value[0] == 't' && value[1] == 'r' && value[2] == 'u' && value[3] == 'e');
//	}
//
