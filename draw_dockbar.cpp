...
static bool dock_paint(dock_s id, rect& client, draw::control** p1, int c1, draw::control** p2, int c2)
{
	...
	{
	case DockLeft:
		draw::splitv(rc.x1, rc.y1, e1.size, rc.height(), dock_ids[id], sx, 64, 400, false);
		client.x1 += e1.size + sx;
		rc.x2 = rc.x1 + e1.size;
		break;
	case DockRight:
		draw::splitv(rc.x2, rc.y1, e1.size, rc.height(), dock_ids[id], sx, 64, 400, true);
		client.x2 -= e1.size + sx;
		rc.x1 = rc.x2 - e1.size;
		break;
	case DockBottom:
		draw::splith(rc.x1, rc.y2, rc.width(), e1.size, dock_ids[id], sx + 1, 64, 400, true);
	...
	{
		draw::splith(rc.x1, rc.y1, rc.width(), e2.size, dock_ids[id + 1], sx, 64, 400, false);
