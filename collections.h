...
{
	arefc() { aref<T>::initialize(); }
	~arefc() { aref<T>::clear(); }
	void*					add(const void* object) override { auto& m = aref<T>::addr(); if(object) m = *static_cast<const T*>(object); return &m; }
	void					clear() override { aref<T>::clear(); }
	void*					get(int index) const override { return aref<T>::data + index; }
	unsigned				getcount() const override { return aref<T>::count; }
