...
{
	return strcmp((*((xsbase**)p1))->id, (*((xsbase**)p2))->id);
}

static int compare_global(const void* p1, const void* p2)
{
	return strcmp((*((xsglobal**)p1))->id, (*((xsglobal**)p2))->id);
}

static void write_ref(io::stream& e, xsbase& m, const char* path = 0)
{
	e << "<a href=\"";
	if(path)
		e << path << "/";
...
static void write_ref(io::stream& e, xsglobal& m, const char* path = 0)
{
	e << "<a href=\"";
	if(path)
		e << path << "/";
	e << m.id << "_glnsp.html\">" << m.id << "</a>";
}

static void write_type(io::stream& e, const xsfield* f)
{
	if(f == number_type)
		e << "number";
	else if(f == text_type)
...
static void write_examples(io::stream& e, xsbase& m)
{
	if(m.count < 2)
		return;
	opentag(e, "section");
...
static void write_header(io::stream& e, xsbase& m, xsdoc& doc)
{
	char temp[256];
	opentag(e, "section");
	opentag(e, "h1");
...
static void write_field(io::stream& e, const xsfield* f)
{
	if(strcmp(f->id, "id") == 0)
		return;
	if(strcmp(f->id, "text") == 0)
...
static void write_syntaxis(io::stream& e, xsbase& m)
{
	if(!m.data)
		return;
	opentag(e, "section");
...
static void write_metaobject(xsbase& m, xsdoc& doc)
{
	char temp[260];
...
static void write_metaobject(xsglobal& m, xsdoc& doc)
{
	char temp[260];
	io::file e(szurl(temp, "help/metadata", m.id, "html", "_glnsp"), StreamWrite | StreamText);
	if(!e)
		return;
	header(e);
	//write_header(e, m, doc);
...
static void write_metadata(io::stream& e, xsdoc& doc)
{
	xsbase* meta[512];
	int count = 0;
	for(auto m = xsbase::first; m; m = m->next)
		meta[count++] = m;
...
static void write_global(io::stream& e, xsdoc& doc)
{
	xsglobal* meta[512];
	int count = 0;
	for(auto m = xsglobal::first; m; m = m->next)
		meta[count++] = m;
	qsort(meta, count, sizeof(meta[0]), compare_global);
	opentag(e, "table");
	for(auto i = 0; i < count; i++)
	{
		write_metaobject(*meta[i], doc);
